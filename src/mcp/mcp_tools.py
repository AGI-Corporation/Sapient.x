...
@register_tool("parcel_get_place_hierarchy", "Return country/region/county/city/parcel IDs for a coordinate.")
...
@register_tool("parcel_get_agent_state", "Get optimization metrics and suggested actions for a parcel agent.")
...
@register_tool("parcel_get_usdx_balance", "Get the USDx (stablecoin) balance and limits for a parcel agent wallet.")
async def tool_get_usdx_balance(parcel_id: str) -> Dict:
    return {
        "parcel_id": parcel_id,
        "balance_usdx": 1500.0,
        "limits": {"daily_transfer": 500.0}
    }


@register_tool("parcel_propose_usdx_incentive_contract", "Propose a USDx-denominated incentive contract.")
...
class MCPToolkit:
    ...
    async def _route_x_call(self, tool_name: str, args: Dict) -> Dict:
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                data = resp.json()
                if "error" in data:
                    return {"success": False, "error": data["error"].get("message", "Unknown RPC error")}
    ...
    def validate_parameters(self, tool_spec: Dict, params: Dict) -> (bool, Optional[str]):
